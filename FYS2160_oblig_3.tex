...
\begin{exercise}
	\begin{itemize}
		...
		\item[e)]
			...
			\begin{align*}
				T = \frac{\Delta \epsilon}{k} \left( \ln  \frac{N}{n}  +\frac{2n}{N} \right) ^{-1}
			\end{align*}
			In the limit when $N>>n$ (i.e. small temperatures), $\ln  \frac{N}{n}  >> \frac{2n}{N}$. In this limit, the temperature is
			\begin{align}
				T &\approx \frac{\Delta \epsilon}{k}\left( \ln \frac{n}{N} \right) ^{-1} \nonumber\\
				\Rightarrow n &\approx Ne^{-\Delta\epsilon/(kT)} \label{eq: crystal number of vacancies n(T)}
			\end{align}
		...
		\item[h)]
			...
			\end{align*}
			Again, using that the total energy needed to make $n$ vacancies is $U$. Then $n=U/\Delta \epsilon$ as above. Eq. \eqref{eq: crystal number of vacancies n(T)} then becomes
			\begin{align*}
				\frac{U}{\Delta \epsilon} &= Ne^{-\Delta \epsilon/(kT)} \\
				U = \Delta \epsilon Ne^{-\Delta \epsilon /(kT)} 
			\end{align*}
			so the expression for the specific heat capacity is
			\begin{align*}
				C_V &= \Delta \epsilon Ne^{-\Delta \epsilon/(kT)} \cdot \frac{\Delta \epsilon}{kT^2} \\
				&= \frac{\Delta \epsilon^2 N}{kT^2}e^{-\Delta \epsilon /(kT)}
			\end{align*}
			A plot of this in the temperature range from $T=0$ to $T=1000$ K is found in fig. \ref{fig: crystal heat capacity}. 
			\begin{figure}
				\centering
